...
class OzakiDerivative:
    ...
    Executes exactly via RNS + WGMMA math using Ozaki Scheme II.
    """
    def __init__(self, block_size=64, halo=4):
        self.bs = block_size
        self.ng = halo  
    ...
    def _ozaki_contract(self, block_in, D_mods, axis):
        ...
        regs_base = scaled_ints[None, :, :] % self.c.mods_base[:, None, None]
        ...
        # Apply matrix contraction depending on axis
        if axis == 0:
            res_raw = jnp.einsum('krc,kcy->kry', D_mods, regs_full)[:, :, self.ng:-self.ng]
        else:
            res_raw = jnp.einsum('krc,koc->kro', regs_full, D_mods)[:, self.ng:-self.ng, :]
            
        res_mods = res_raw % self.c.mods_full[:, None, None]
    ...
    def _apply_pipeline(self, u, D_mods, divisor, axis):
        """Handles grid patching, compiling the Ozaki math, and unpatching."""
        nx, ny = u.shape
        ...
        # 1. Pad for the stencil halo
        u_pad = jnp.pad(u_aligned, ((self.ng, self.ng), (self.ng, self.ng)), mode='edge')
        
        # 2. Extract Patches
        patches = lax.conv_general_dilated_patches(
            u_pad[None, :, :, None], (patch_size, patch_size), (self.bs, self.bs), padding="VALID",
        ...
        out_aligned = computed_patches.reshape(nbx, nby, self.bs, self.bs).transpose(0, 2, 1, 3).reshape(nbx * self.bs, nby * self.bs)
        
        # Slice off the block alignment padding so it perfectly matches the input size
        return out_aligned[:nx, :ny]

    # API Hooks to match SpatialDerivative
    ...
    def compute_ko(self, u, dx, sigma, axis):
        # Apply the 256.0 divisor and KO sigma scaling
