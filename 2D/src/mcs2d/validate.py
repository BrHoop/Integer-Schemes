"""
Baseline validation + characterization harness for the MCS 2D solver (Step 1.1).

This module is the research-meeting face of the validation suite: every function
here either (a) certifies a property of the discretization or (b) produces a
figure you can drop into a slide.  The machine-checked CI guards live in
`2D/tests/validation/` and import the analysis helpers from here so the two never
drift.

Design
------
Almost every spectral diagnostic is post-processing of ONE object: the linearized
semi-discrete symbol M(kx, ky) (a 10x10 complex matrix per wavenumber) and its
real-space counterpart, the Jacobian J = d(RHS)/d(state).  Once you have those,
stability, dispersion error, KO kernel shape, mode counting, the CFJ tachyon,
the RK4 amplification factor, and strong hyperbolicity are all cheap functions of
the same eigenvalues.  So the heavy lifting (`_symbol`, `_build_jacobian`)
happens once and everything else is a view.
...
CPU/GPU
-------
Everything except `profile_baseline` runs on CPU (it is all small-matrix linear
algebra and short evolutions).  Profiling is GPU-only and refuses to run on CPU.
"""
...
# ── Field indices (match MaxwellChernSimons2D) ────────────────────────────────
EX, EY, EZ = 0, 1, 2
BX, BY, BZ = 3, 4, 5
XI, PI     = 6, 7
PSI, PHI   = 8, 9
...
# Deliverables live next to the Step 1.1 doc so figures + spec stay together.
# validate.py is 2D/src/mcs2d/validate.py -> repo root is parents[3].
_RESULTS_DIR = str(Path(__file__).resolve().parents[3]
                   / "docs/phases/phase_0_2d_foundation/step_0.1_results")


# ══════════════════════════════════════════════════════════════════════════════
#  Full 10-field analytic oracle
# ══════════════════════════════════════════════════════════════════════════════

class FullBirefringentOracle:
    """Exact 10-field solution of the 2.5D left-circularly-polarized birefringent
    wave (omega_minus branch), matching `InitialData._birefringent_wave`.

    The EM fields propagate at omega = sqrt(k^2 + m_cs*k); xi grows linearly
    (xi = -cs*Pi_0*t, spatially uniform); Pi is constant at Pi_0; Psi = Phi = 0.
    This is the only configuration of the MCS system with a closed-form solution,
    so it is the gold standard for every accuracy check.
    """
    ...
    def __init__(self, params: Dict[str, Any]):
        ...
        self.e1x = -self.ky / self.k
        self.e1y = self.kx / self.k

    def state(self, X, Y, t) -> np.ndarray:
        """Return the exact (10, *X.shape) field stack at time t."""
        X = np.asarray(X); Y = np.asarray(Y)
        phase = self.kx * X + self.ky * Y - self.omega * t
        cosф = np.cos(phase)
        Ex = self.E0 * self.e1x * cosф
        Ey = self.E0 * self.e1y * cosф
        Ez = self.E0 * np.sin(phase)
        Bx = (self.ky * Ez) / self.omega
        By = (-self.kx * Ez) / self.omega
        Bz = (self.kx * Ey - self.ky * Ex) / self.omega
        out = np.zeros((NF,) + X.shape, dtype=np.float64)
        out[EX], out[EY], out[EZ] = Ex, Ey, Ez
        out[BX], out[BY], out[BZ] = Bx, By, Bz
    ...
    def Ez(self, X, Y, t):
        return self.E0 * np.sin(self.kx * np.asarray(X) + self.ky * np.asarray(Y)
                                - self.omega * t)


# ══════════════════════════════════════════════════════════════════════════════
#  6th-order finite-difference Fourier symbols
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  Linearized semi-discrete symbol  M(kx, ky)
# ══════════════════════════════════════════════════════════════════════════════

def symbol_params(params: Dict[str, Any], dx: float, dy: float) -> Dict[str, float]:
...
def _symbol(kx: float, ky: float, p: Dict[str, float],
            *, with_ko: bool = True, with_cs: bool = True,
            principal_only: bool = False) -> np.ndarray:
    """The 10x10 complex semi-discrete symbol of the MCS operator, linearized at
    the homogeneous Pi = Pi_0 background.
    ...
    if principal_only:
        sx = 1j * kx
        sy = 1j * ky
        qx = qy = 0.0          # scalar sector handled separately
        mc = 0.0
        K1 = K2 = 0.0
        ko = 0.0
    else:
    ...
    # Maxwell curl + constraint-cleaning gradients + CS mass term (-mc on E<-B).
    ...
    # Scalar (xi, Pi) wave sector: xi_t = -cs Pi ; Pi_t = -cs (d2x+d2y) xi.
...
def _build_jacobian(nx: int = 16, ny: int = 16, *,
                    params_file: str = _PARAMS_FILE,
                    ko_sigma: float = 0.05,
                    lam: float = 0.4,
                    background: str = "pi0") -> Tuple[np.ndarray, "MaxwellChernSimons2D", Dict]:
    """Exact Jacobian of the (periodic) unfused RHS over the interior DOFs.
    ...
    Returns (J, sim, params) with J of shape (10*nx*ny, 10*nx*ny).
    """
    ...
    sim = MaxwellChernSimons2D(dx, dy, lam, params)
    ng = sim.ng

    if background == "pi0":
        cs = params.get("enable_cs", 1.0)
        Pi0 = (lam * 2.0) / (2.0 * cs * lam)
        u0 = np.zeros((NF, nx, ny)); u0[PI] = Pi0
    else:
        u0 = np.zeros((NF, nx, ny))
    u0_flat = jnp.asarray(u0.reshape(-1))

    def rhs_interior(u_flat):
        interior = u_flat.reshape(NF, nx, ny)
        data = jnp.zeros((NF, nx + 2 * ng, ny + 2 * ng))
        data = data.at[:, ng:ng + nx, ng:ng + ny].set(interior)
        out = sim._rhs_unfused(WaveState(data)).data
        return out[:, ng:ng + nx, ng:ng + ny].reshape(-1)
...
def run_sim(scheme: str, nx: int, ny: int, n_steps: int,
            *, params_file: str = _PARAMS_FILE, **overrides):
    """Build a birefringent/periodic sim and advance it n_steps under jit+scan.
...
def field_l2_errors(sim, state, params, t) -> Dict[str, float]:
    ...
    exact = oracle.state(X, Y, t)                         # (10, nx, ny)
    num = np.asarray(get_physical(state.data, sim.ng))    # (10, nx, ny)
...
def em_wave_omega(kx: float, p: Dict[str, float], *, fd: bool = True) -> float:
    """Largest |Im(eigenvalue)| of the symbol at (kx, 0) = the birefringent EM
    wave frequency.  fd=True uses the discrete FD symbol; fd=False the continuum
    operator (i*k, -k^2), i.e. the exact PDE dispersion."""
    if fd:
        M = _symbol(kx, 0.0, p, with_ko=False)
    else:
        # Continuum: substitute exact symbols by faking dx->0 via direct build.
        M = _symbol_continuum(kx, 0.0, p)
    return float(np.max(np.abs(np.linalg.eigvals(M).imag)))


def _symbol_continuum(kx: float, ky: float, p: Dict[str, float]) -> np.ndarray:
    ...
    sx, sy = 1j * kx, 1j * ky
    qx, qy = -kx * kx, -ky * ky
    cs, K1, K2, mc = pc["cs"], pc["K1"], pc["K2"], pc["mc"]
    M[EX, BZ] += sy;  M[EX, PSI] += -sx;  M[EX, BX] += -mc
    M[EY, BZ] += -sx; M[EY, PSI] += -sy;  M[EY, BY] += -mc
    M[EZ, BY] += sx;  M[EZ, BX] += -sy;   M[EZ, BZ] += -mc
    M[BX, EZ] += -sy; M[BX, PHI] += sx
    M[BY, EZ] += sx;  M[BY, PHI] += sy
    M[BZ, EY] += -sx; M[BZ, EX] += sy
    M[XI, PI] += -cs;  M[PI, XI] += -cs * (qx + qy)
    M[PSI, EX] += -sx; M[PSI, EY] += -sy; M[PSI, PSI] += -K1
    M[PHI, BX] += sx;  M[PHI, BY] += sy;  M[PHI, PHI] += -K2
    return M


def mode_classification(kx: float, ky: float, p: Dict[str, float],
                        tol: float = 1e-9) -> Dict[str, int]:
    """Count symbol eigenvalues by type at (kx, ky): oscillatory (Re~0),
    ...
    eig = np.linalg.eigvals(_symbol(kx, ky, p, with_ko=False))
    re = eig.real
    return {
...
def principal_symbol_condition(p: Dict[str, float], n_dirs: int = 64) -> float:
    ...
    principal symbol over propagation directions n-hat.  Bounded <=> strongly
    hyperbolic (well-posed).  Evaluated on the 8 first-order fields
    {E, B, Psi, Phi}; the (xi, Pi) sector is a standard scalar wave equation and
    is handled separately."""
    idx = np.array([EX, EY, EZ, BX, BY, BZ, PSI, PHI])
    worst = 0.0
    for theta in np.linspace(0, np.pi, n_dirs):
        nx, ny = np.cos(theta), np.sin(theta)
        # Principal symbol P(n) = i*(A^x nx + A^y ny); use unit |k| = 1.
        M = _symbol(nx, ny, p, principal_only=True)[np.ix_(idx, idx)]
        w, V = np.linalg.eig(M)
...
def energy_spectrum_offmode(sim, state, params) -> float:
    ...
    a single Fourier mode (kx0, ky0); because MCS is linear, ALL power must stay
    in that mode.  Power leaking to harmonics signals aliasing, ghost-cell
    ...
    for ix in (1, nx - 1):
        for iy in (1, ny - 1):
            on += P[ix, iy]
    total = float(np.sum(P)) - float(P[0, 0])   # drop the DC (Pi/xi offset)
    off = total - on
...
def _converge(resolutions, *, ko, dt=None, cfl=None, t_phys=None,
    ...
    """Fit convergence order per field on the unfused scheme, holding PHYSICAL
    TIME fixed across resolutions (the honest protocol -- a fixed step count makes
    T ∝ dx and inflates the order by ~1; see TestOrderSeparation).
    ...
    Uses `floating_point` because step_rk4 honours an explicit dt only there.
    """
    Ns = list(resolutions)
    errs = {f: [] for f in fields}
    for N in Ns:
        sim, st0, params = run_sim("floating_point", N, N, 0, ko_sigma=ko)
...
def convergence_study(resolutions=(24, 48, 96),
    ...
    """Honest fixed-time convergence, as the 'three-line story':
      ...
      2. PRODUCTION (KO on, CFL=0.05, fixed physical time): what you actually get
         -- KO-limited to ~5 because the σ/dx·6th-difference KO is an O(h⁵)
         dissipation (the standard 6th-order KO is 'rated' for 4th-order FD; on
         6th-order FD it caps convergence one order below the stencil).
      3. TEMPORAL FLOOR (KO on, CFL=0.4): RK4 is 4th-order in time; at large CFL
         the temporal error dominates -> ~4.  At the operating CFL=0.05 dt is so
         small this floor never binds (production stays KO-limited at ~5).

    Earlier reports of ~6.5 were a fixed-step-count artifact (T ∝ dx inflates the
    order); everything here holds physical time fixed.
    """
    os.makedirs(outdir, exist_ok=True)
    plt = _matplotlib()

    Ns, errs_st, ord_st = _converge(resolutions, ko=0.0, dt=0.001)             # stencil
    _,  errs_pr, ord_pr = _converge(resolutions, ko=0.05, cfl=0.05, t_phys=0.5)  # production
    _,  errs_tf, ord_tf = _converge(resolutions, ko=0.05, cfl=0.40, t_phys=0.5,
                                    fields=("Ez",))                            # temporal floor

    # Figure: Ez convergence under all three regimes, with reference slopes.
    ...
    ax.set_title("MCS spatial convergence (fixed physical time):\n"
...
def spectral_analysis(outdir: str = "validation_plots",
    ...
    for kx in kxs:
        for ky in kys:
            eig_ko.extend(np.linalg.eigvals(_symbol(kx, ky, p, with_ko=True)))
            eig_noko.extend(np.linalg.eigvals(_symbol(kx, ky, p, with_ko=False)))
    ...
    ax.set_title(f"Semi-discrete spectrum (Lambda={lam})")
    ...
    ax.set_title("Numerical vs analytic MCS dispersion")
    ax.legend(); ax.grid(True, alpha=0.3)
    ...
    shift = []
    for k in kline:
        re_ko = np.linalg.eigvals(_symbol(k, 0.0, p, with_ko=True)).real
        re_no = np.linalg.eigvals(_symbol(k, 0.0, p, with_ko=False)).real
    ...
    ax.set_title("Kreiss-Oliger dissipation kernel")
    ax.legend(); ax.grid(True, alpha=0.3)
...
def stability_monitor(scheme: str = "fused_floating_point",
                      n_steps: int = 500, record_every: int = 25,
                      nx: int = 64, outdir: str = "validation_plots",
    ...
    os.makedirs(outdir, exist_ok=True)
    plt = _matplotlib()
    from mcs2d.main import calc_constraints
    if not _is_gpu() and n_steps > 800:
        n_steps = 800
    sim, state0, params = run_sim(scheme, nx, nx, 0, Lambda=lam)
    oracle = FullBirefringentOracle(params)
    ...
    fig.suptitle(f"Stability monitor ({scheme}, Lambda={lam}, {n_steps} steps)")
...
def spectrum_table(outdir: str = "validation_plots",
    ...
    pf = symbol_params({**params, "Lambda": 0.4}, 0.3125, 0.3125)
    m_cs = pf["mc"]
    below = mode_classification(0.5 * m_cs, 0.0, pf)
    above = mode_classification(3.0 * m_cs, 0.0, pf)
    ...
    # Gundlach oscillator), so the most-damped eigenvalue tracks -K/2.  Evaluated
    # at a CFJ-STABLE wavenumber (|k| > m_cs) so no tachyon contaminates it.
    ...
    for K in (0.0, 0.5, 1.0, 2.0, 5.0):
        pk = symbol_params({**params, "Lambda": 0.4, "K1": K, "K2": K,
                            "ko_sigma": 0.0}, 0.3125, 0.3125)
        eig = np.linalg.eigvals(_symbol(2.0, 2.0, pk, with_ko=False))
...
def profile_baseline(outdir: str = "validation_plots"):
    """GPU-only throughput baseline (thin wrapper over benchmark.py).

    benchmark.main() is argv-driven (parfile, out_dir); set argv so its figures
    and CSV land in `outdir`, then restore it."""
    if not _is_gpu():
        raise RuntimeError("profile_baseline requires a GPU (run on the H200). "
                           "CPU profiling is meaningless for this project.")
    import sys
    from mcs2d import benchmark
    os.makedirs(outdir, exist_ok=True)
    saved = sys.argv
    try:
        sys.argv = ["benchmark", _PARAMS_FILE, outdir]
        benchmark.main()
    finally:
        sys.argv = saved
...
def main(outdir: str = _RESULTS_DIR):
    ...
    Produces, in `outdir`:
      convergence_spatial.png, spectrum_{complex_plane,dispersion,ko_kernel,
      cfl_margin}.png, stability_monitor.png, spectrum_table.txt, and
      validation_summary.txt (the numeric deliverable: fitted orders + certs).
    Profiling (GPU-only) is appended when run on the H200.
    """
    os.makedirs(outdir, exist_ok=True)
    ...
    lines = ["MCS 2D — Step 1.1 validation summary", "=" * 38, "",
             ...
             *[f"        {f:3s}: p = {pr[f]:.2f}" for f in ("Ex", "Ey", "Ez", "Bx", "By", "Bz")],
             ...
             "  (stencil is 6th order; default 6th-order KO caps production at ~5th;",
             "   RK4's 4th-order floor never binds at the operating CFL=0.05.)",
             "",
    ...
    if _is_gpu():
        print(">> GPU profiling ..."); profile_baseline(outdir=outdir)
    else:
        print(">> Skipping profiling suite (CPU detected; run on the H200).")
